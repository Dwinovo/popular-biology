...
public final class InitRegistry {
    ...
    public static final ResourceKey<Registry<IPetJob>> PET_JOB_KEY = ResourceKey.createRegistryKey(
        ResourceLocation.fromNamespaceAndPath(PopularBiology.MODID, "pet_jobs")
    );

    public static final Registry<IPetJob> PET_JOB_REGISTRY = new RegistryBuilder<>(PET_JOB_KEY)
        .sync(true)
        .defaultKey(ResourceLocation.fromNamespaceAndPath(PopularBiology.MODID, "none"))
        .create();

    public static final DeferredRegister<IPetJob> PET_JOBS = DeferredRegister.create(PET_JOB_REGISTRY, PopularBiology.MODID);
